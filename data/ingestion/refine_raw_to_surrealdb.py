...
def build_domain_upsert_query(domain_name: str, records: list[dict[str, object]]) -> str:
    """Menyusun query refresh koleksi clean domain di SurrealDB.

    Args:
        domain_name: Nama domain clean target.
        records: Data clean yang akan disimpan.

    Returns:
        Query SQL SurrealDB untuk menghapus dan mengisi ulang koleksi domain.
    """

    statements = [
        f"DEFINE TABLE IF NOT EXISTS clean_{domain_name} SCHEMALESS;",
        f"DELETE clean_{domain_name};",
    ]
...
def send_to_surrealdb(domain_name: str, records: list[dict[str, object]]) -> None:
    """Mengirim record clean ke SurrealDB menggunakan endpoint SQL.
    ...
    query = build_domain_upsert_query(domain_name, records)
    headers = {
